...
class TradingChart:
    ...
    def _render_net_worth(self, step_range, times, current_step, net_worths, benchmarks):
        ...
        last_time = self.df['OpenTime'].values[current_step]
    ...
    def render(self, current_step, net_worths, benchmarks, trades, window_size=200):
        ...
        times = self.df['OpenTime'].values[step_range]
        ...
        date_col = pd.to_datetime(self.df['OpenTime'], unit='s').dt.strftime('%m/%d/%Y %H:%M')
